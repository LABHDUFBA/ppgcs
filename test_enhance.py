#!/usr/bin/env python3
"""
Test version of the enhancer - processes only first 3 entries for quick testing.
"""
...
def enhance_faculty_with_scholar(faculty_data: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """
    Enhance each faculty member with Google Scholar data (limited to first 3 for testing).
    ...
    test_data = faculty_data[:3]
    enhanced_data = []
    
    print(f"Processing {len(test_data)} faculty members (test run)...")
    
    for i, faculty in enumerate(test_data, 1):
        ...
        print(f"[{i}/{len(test_data)}] Processing: {name}")
    ...
    # Add the rest of the data without enhancement (for testing)
    enhanced_data.extend(test_data)
    enhanced_data.extend(faculty_data[3:])
...
def main():
    """Main function."""
    print("=" * 60)
    print("Testing Faculty Data Enhancement with Google Scholar")
    print("=" * 60)
    
    # Paths
    input_file = Path('data/processed/faculty_data.json')
    output_file = Path('data/processed/faculty_data_enhanced_test.json')
    ...
    # Count successful matches in the test set
    test_set = enhanced_data[:3]
    successful = sum(1 for f in test_set if f.get('google_scholar', {}).get('total_citations', 0) > 0)
    print(f"Successfully matched {successful}/{len(test_set)} faculty members on Google Scholar")
    
    # Show some stats from test set
    if test_set:
        total_citations = sum(f.get('google_scholar', {}).get('total_citations', 0) for f in test_set)
        print(f"Total citations in test set: {total_citations}")
